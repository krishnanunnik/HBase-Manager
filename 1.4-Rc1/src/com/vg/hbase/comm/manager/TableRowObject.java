...
public class TableRowObject {
	...
	private boolean passDateFilter() {
		...
		if (HBaseManagerViewTable.isDateFilterSet()) {

			pass = false;

			String cq = HBaseManagerViewTable.getDateFilterOnCq();

			long dateFrom = HBaseManagerViewTable.getFilterDateFrom();
			long dateTo = HBaseManagerViewTable.getFilterDateTo();
			long resultDate = 0;

			byte[] resultVal = this.resultOb.getValue(Bytes.toBytes(colFamily), Bytes.toBytes(cq));
			if (resultVal != null) {
				resultDate = Bytes.toLong(resultVal);

				if (resultDate >= dateFrom && resultDate <= dateTo) {
					pass = true;
					HBaseManagerViewTable.coloumnTypeList.put(cq, "DATEANDTIME");
				}
			}
