...
import com.vg.hbase.operations.base.LongComparator;
...
public class HBaseManagerViewTable extends javax.swing.JFrame {
	...
	public static void addTableList() {
		...
		clearTogglePane();
		try {
			DefaultComboBoxModel model = (DefaultComboBoxModel) comboTableList
					.getModel();
	...
	/*
	 * private int[] getPositionArray(String[] colData, int listSize,
	 * DefaultTableModel tm) { int posArray[] = new int[colData.length]; // int
	 * position = -1; for (int i = 0; i < colData.length; i++) {
	 * 
	 * for (int j = 0; j < listSize; j++) { if
	 * (colData[i].equals(tm.getColumnName(j))) { posArray[i] = j; break; } } }
	 * 
	 * return posArray; }
	 */
	...
	private String[][] getRowData(int row) {
		...
		for (int i = 0; i < coloumnList.size(); i++) {
			...
			if (StringUtils.isEmpty(data)) {
				...
				new HbaseManagerTableGetter(tablename)
						.deleteTableColoumnQualifier(rowKey, colname, family);
				continue;
			}
	...
	private void deleteHbaseRow(String rowKey) {
		...
		new HbaseManagerTableGetter((String) comboTableList.getSelectedItem())
				.deleteTableRow(Bytes.toBytes(rowKey));
	...
	private void initComponents() {
		...
		controlPane.setBorder(javax.swing.BorderFactory
				.createLineBorder(new java.awt.Color(0, 0, 0)));
		...
		javax.swing.GroupLayout panelInternalCutomizeLayout = new javax.swing.GroupLayout(
				panelInternalCutomize);
		panelInternalCutomize.setLayout(panelInternalCutomizeLayout);
		panelInternalCutomizeLayout
				.setHorizontalGroup(panelInternalCutomizeLayout
						.createParallelGroup(
		...
		panelInternalCutomizeLayout
				.setVerticalGroup(panelInternalCutomizeLayout
						.createParallelGroup(
								javax.swing.GroupLayout.Alignment.LEADING)
						.addGroup(
								panelInternalCutomizeLayout
		...
		comboCompareOpLsit.setModel(new javax.swing.DefaultComboBoxModel(
				new String[] { "EQUAL", "GREATER", "LESS", "GREATER OR EQUAL",
		...
		javax.swing.GroupLayout panelInternalSetFiltersLayout = new javax.swing.GroupLayout(
				panelInternalSetFilters);
		...
		javax.swing.GroupLayout panelInteranlCustomSetLayout = new javax.swing.GroupLayout(
				panelInteranlCustomSet);
		panelInteranlCustomSet.setLayout(panelInteranlCustomSetLayout);
		panelInteranlCustomSetLayout
				.setHorizontalGroup(panelInteranlCustomSetLayout
						.createParallelGroup(
								javax.swing.GroupLayout.Alignment.LEADING)
						.addGroup(
								javax.swing.GroupLayout.Alignment.TRAILING,
								panelInteranlCustomSetLayout
										...
										.addGap(85, 85, 85)));
		panelInteranlCustomSetLayout
				.setVerticalGroup(panelInteranlCustomSetLayout
						.createParallelGroup(
								javax.swing.GroupLayout.Alignment.LEADING)
						.addGroup(
								panelInteranlCustomSetLayout
		...
		javax.swing.GroupLayout controlPaneLayout = new javax.swing.GroupLayout(
				controlPane);
		...
		userTable.setBorder(javax.swing.BorderFactory
				.createLineBorder(new java.awt.Color(0, 0, 0)));
		...
		menuNewConnection.setAccelerator(javax.swing.KeyStroke.getKeyStroke(
		...
		menuOpenAdmin.setAccelerator(javax.swing.KeyStroke.getKeyStroke(
	...
	public void clickVersionActionPerformed(ActionEvent evt) {
		String rowKey = (String) this.userTable.getModel().getValueAt(
				this.userTable.getSelectedRow(), 0);
		new HbaseManagerColoumnVersionDetailView(rowKey,
				(String) comboTableList.getSelectedItem(),
				(String) comboFamilyList.getSelectedItem()).setVisible(true);
	...
	private void clickAddFilterToListActionPerformed(
	...
	private void clickSaveActionPerformed(java.awt.event.ActionEvent evt) {
		...
		for (int i = 0; i < selectedRow.length; i++) {
			...
			if (StringUtils.isNotEmpty(rowKey)) {
				...
				new HbaseManagerTableGetter(
						(String) comboTableList.getSelectedItem())
						.insertRowToDb(tableData,
	...
	private void clickRefreshTableDataActionPerformed(
			java.awt.event.ActionEvent evt) {
		addTableList();
	}
	...
	private void clickNEWCQActionPerformed(java.awt.event.ActionEvent evt) {
		...
		String str = JOptionPane.showInputDialog(null,
				"New Coloumn Qualifier Name : ", "HbaseManager : VG", 1);
		tm.addColumn(str);
		coloumnList.add(str);
	...
	private void clickScanActionPerformed(java.awt.event.ActionEvent evt) {
		...
		this.setTitle("HBase Manager: Action- Scan Table: "
				+ comboTableList.getSelectedItem() + " Family: "
				+ comboFamilyList.getSelectedItem());
		coloumnList.clear();
			...
			if (mannualRange) {
				...
				if (FILTER_SET) {
					...
					System.out.print("Filters; "
							+ newFilterList.getFilters().isEmpty());
		...
		System.out.println("\nPagination Count :" + paginationCount
				+ "\nPaginatorSize :" + paginateBackWardLimits.size() + " , "
				+ paginateForwardLimits.size());

		ResultScanner resultScan = HbaseManagerTableGetter.getList(
				CURRENT_START_RANGE, CURRENT_STOP_RANGE,
				Bytes.toBytes(colFamily), null, currentRowLimit, newFilterList,
				tblName);
		...
		for (Result resultObj : resultScan) {
			...
			TableRowObject rowOb = new TableRowObject(resultObj, colFamily,
					false);
		...
		System.out.println("Rendered @:" + tableModel.getRowCount()
				+ " Rows , Limit was to @: " + currentRowLimit);
		...
		if (paginationCount < 0) {
			...
			if (PAGINATED_ALREADY) {
				System.out
						.println("No more Previous records, Restting paginators and Limits");
		...
		labelTotalRows.setText("Retrieved " + totalRowsBrought + " rows of "
				+ comboTableList.getSelectedItem());
		...
		if (tableModel.getRowCount() > 0) {
			...
			labelLoadingData.setText("Table Empty. No rows to fetch");

		}
	}

	private boolean setRowData(int row, String[][] tblData,
	...
	private void clickRemoveFilterFromListActionPerformed(
			java.awt.event.ActionEvent evt) {
		DefaultListModel lmodel = (DefaultListModel) listFiltersSet.getModel();
	...
	private void menuOpenTableDesignerActionPerformed(
			java.awt.event.ActionEvent evt) {
	...
	private void clickPaginateBackwardActionPerformed(
			java.awt.event.ActionEvent evt) {
		...
		System.out.println("Setting Forward: Start @ :" + CURRENT_START_RANGE
				+ " & " + "Stop @: " + CURRENT_STOP_RANGE);
		clickScanActionPerformed(evt);

	}

	private void clickPaginateForwardActionPerformed(
			java.awt.event.ActionEvent evt) {
		...
		if (paginationCount == -1) {
			...
			System.out.println("Setting Forward: Start @ :"
					+ CURRENT_START_RANGE + " & " + "Stop @: "
					+ CURRENT_STOP_RANGE);
	...
	private void valueStartRowInputMethodTextChanged(
			java.awt.event.InputMethodEvent evt) {
	...
	public void convertColoumnstoUserData(HashMap<String, String> actionMap) {
		...
		System.out.println("Need to Convert " + noColoumnstoConvert
				+ "to User Types");
	...
	public static void main(String args[]) {
		...
		try {
			...
			java.util.logging.Logger.getLogger(
					HBaseManagerViewTable.class.getName()).log(
	...
	private void populateColoumnList() {
		...
		String tblName = (String) comboTableList.getSelectedItem();
		HBaseTableManager tblmngr = HbaseManagerTableGetter.getTblManager();
		ResultScanner resultScan = HbaseManagerTableGetter.getList("0", "zzz",
				Bytes.toBytes(colFamily), null, currentRowLimit,
				new FilterList(), tblName);
	...
	private FilterList obtainFilterList() {
		...
		byte[] family = Bytes.toBytes((String) comboFamilyList
				.getSelectedItem());
		...
		if (dateFilterSet) {
			...
			System.out.println("Applying date filters");
			valFilter = new SingleColumnValueFilter(family,
					Bytes.toBytes(dateFilterOnCq), CompareOp.GREATER_OR_EQUAL,
					new LongComparator(filterDateFrom));
			valFilter.setFilterIfMissing(true);
			filters.addFilter(valFilter);

			valFilter1 = new SingleColumnValueFilter(family,
					Bytes.toBytes(dateFilterOnCq), CompareOp.LESS_OR_EQUAL,
					new LongComparator(filterDateTo));
		...
		for (int i = 0; i < listFiltersSet.getModel().getSize(); i++) {
			...
			System.out.println("Filter String:" + val + "Searching:" + vals[1]
					+ " for :" + vals[2]);
			...
			if (vals[1].equals("EQUAL")) {
				...
				valFilter = new SingleColumnValueFilter(family,
						Bytes.toBytes(vals[0]), CompareOp.EQUAL,
				...
				valFilter = new SingleColumnValueFilter(family,
						Bytes.toBytes(vals[0]), CompareOp.NOT_EQUAL,
						Bytes.toBytes(vals[2]));
				valFilter.setFilterIfMissing(true);
				filters.addFilter(valFilter);
				...
				valFilter = new SingleColumnValueFilter(family,
						Bytes.toBytes(vals[0]), CompareOp.GREATER,
						Bytes.toBytes(vals[2]));
				filters.addFilter(valFilter);
				...
				valFilter = new SingleColumnValueFilter(family,
						Bytes.toBytes(vals[0]), CompareOp.LESS,
						Bytes.toBytes(vals[2]));
				filters.addFilter(valFilter);
				...
				valFilter = new SingleColumnValueFilter(family,
						Bytes.toBytes(vals[0]), CompareOp.GREATER_OR_EQUAL,
						Bytes.toBytes(vals[2]));
				filters.addFilter(valFilter);
			...
			if (vals[1].equals("LESSER OR EQUAL")) {
				valFilter = new SingleColumnValueFilter(family,
						Bytes.toBytes(vals[0]), CompareOp.LESS_OR_EQUAL,
						Bytes.toBytes(vals[2]));
				filters.addFilter(valFilter);
				...
				valFilter = new SingleColumnValueFilter(family,
						Bytes.toBytes(vals[0]), CompareOp.NO_OP,
						Bytes.toBytes(vals[2]));
				filters.addFilter(valFilter);
	...
	private String getConvertedValue(String currentVal, String action) {
		...
		if (action.equals("DATE")) {
			...
			convertedValue = new Date(Long.parseLong(currentVal))
					.toLocaleString();
	...
	public static void setFilterDateFrom(long filterDateFrom) {
		...
		dateFilterOnCq = (String) comboFieldList.getSelectedItem();
	...
	private void clickSetDateFilterActionPerformed(ActionEvent evt) {
		...
		if (dateFilterSet) {
			...
			if (confirm == JOptionPane.YES_OPTION) {
				...
				clickSetDateFilter.setText("Set Date Filter");
				dateFilterOnCq = "";
				JOptionPane.showMessageDialog(this, "Date Filter Deactivated",
						"Information", JOptionPane.INFORMATION_MESSAGE);
			} else {
				JOptionPane.showMessageDialog(this, "Date Filter Still Active",
