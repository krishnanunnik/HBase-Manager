...
public class HBaseTableManager {
	...
	if (!resultObject.isEmpty()) {
	    ...
	    NavigableMap<byte[], NavigableMap<byte[], NavigableMap<Long, byte[]>>> resultVersionMap = resultObject
		    .getMap();
    ...
    /*
     * public List<String[][]> getDataFillerByKeyValueVersions(Result
     * resultObject) { List<String[][]> dataList = null;
     * 
     * if (!resultObject.isEmpty()) {
     * 
     * TableRowObject row = new TableRowObject(resultObject); dataList =
     * row.getColoumnData(); this.columnQualifiers = row.getColoumns(); //
     * row.resetRowObject();
     * 
     * }
     * 
